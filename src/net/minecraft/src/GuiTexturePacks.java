...
public class GuiTexturePacks extends GuiScreen {
    ...
    private void loadTexturePackMetadata() {
        ...
        // Step 1: Add Default pack to the list (do NOT apply it yet)
        PackInfo defaultPack = new PackInfo("Default", new File(System.getProperty("user.dir") + "/texturepacks"));
        defaultPack.description = "The original look of Minecraft.";
        texturePackList.add(defaultPack);
        selectedIndex = 0; // Default is initially selected
    ...
    private void loadPackDescription(PackInfo pack) {
        try {
            ...
            if (pack.file.isDirectory()) {
                File descFile = findFileIgnoreCase(pack.file, "pack.txt");
                if (descFile != null) pack.description = readFile(descFile);
            } else if (pack.file.getName().endsWith(".zip")) {
                try (ZipFile zip = new ZipFile(pack.file)) {
                    ZipEntry descEntry = findZipEntryIgnoreCase(zip, "pack.txt");
    ...
    private File findFileIgnoreCase(File dir, String name) {
        ...
        for (File f : dir.listFiles()) {
            if (f.isDirectory()) {
                File result = findFileIgnoreCase(f, name);
                if (result != null) return result;
            } else if (f.getName().equalsIgnoreCase(name)) return f;
        }
        return null;
    }

    private ZipEntry findZipEntryIgnoreCase(ZipFile zip, String name) {
        return zip.stream().filter(entry -> entry.getName().equalsIgnoreCase(name)).findFirst().orElse(null);
    }
            ...
            if (pack.textureID == -1) {
                ...
                try {
                    if (pack.file != null && pack.file.isDirectory()) {
                        File iconFile = findFileIgnoreCase(pack.file, "pack.png");
                        if (iconFile != null) {
                            img = ImageIO.read(iconFile);
                        }
                    } else if (pack.file != null && pack.file.getName().endsWith(".zip")) {
                        try (ZipFile zip = new ZipFile(pack.file)) {
                            ZipEntry iconEntry = findZipEntryIgnoreCase(zip, "pack.png");
                            if (iconEntry != null) img = ImageIO.read(zip.getInputStream(iconEntry));
                        }
                    }

                    // Fallback for Default pack if no pack.png found
                    if (img == null && pack.name.equals("Default")) {
                        // Take a 64x64 block from Minecraft texture atlas as a placeholder
                    ...
                    if (img != null) {
                        ...
                        // Resize to PREVIEW_SIZE with alpha
