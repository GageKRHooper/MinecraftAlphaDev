...
public class EntityRenderer {
    ...
    // Zoom fields
    private float zoomLevel = 0.0F; // 0 = normal, 1 = fully zoomed
    ...
    public void updateRenderer() {
        ...
        float var1 = this.mc.theWorld.getBrightness(
    ...
    public void getMouseOver(float var1) {
        if(this.mc.thePlayer != null) {
            ...
            List var9 = this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(
                this.mc.thePlayer,
                this.mc.thePlayer.boundingBox.addCoord(var7.xCoord * var2, var7.yCoord * var2, var7.zCoord * var2)
            );
            double var10 = 0.0D;
    ...
    private float getFOVModifier(float var1) {
        ...
        // Smooth zoom interpolation: 0 = normal, 1 = fully zoomed
        float targetZoom = var2.isZooming ? 1.0F : 0.0F;
        zoomLevel += (targetZoom - zoomLevel) * 0.1F;

        // Reduce FOV proportionally when zoomed
        baseFOV *= (1.0F - 0.8F * zoomLevel); // 0.8 = max zoom reduction

        if(var2.isInsideOfMaterial(Material.water)) baseFOV = 60.0F * (1.0F - 0.8F * zoomLevel);
        if(var2.health <= 0) {
            float var4 = (float)var2.deathTime + var1;
            baseFOV /= (1.0F - 500.0F / (var4 + 500.0F)) * 2.0F + 1.0F;
        }
    ...
    public void updateCameraAndRender(float var1) {
        if(!Display.isActive()) {
            if(System.currentTimeMillis() - this.prevFrameTime > 500L) this.mc.displayInGameMenu();
        ...
        if(this.mc.inGameHasFocus) {
            ...
            // --- Smooth zoom mouse sensitivity ---
            float zoomFactor = 1.0F - 0.7F * zoomLevel; // sensitivity reduced when zoomLevel = 1
            var3 *= zoomFactor;

            float var4 = (float)this.mc.mouseHelper.deltaX * var3;
            float var5 = (float)this.mc.mouseHelper.deltaY * var3;
            int var6 = (this.mc.options.invertMouse) ? -1 : 1;
